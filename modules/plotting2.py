...
def plot_coefficients(regression_results):
    for name,regression_result in regression_results.items():
        ...
        N = len(subindicies)-1
        fig = plt.figure(figsize=(5/2 * N/2,5))
        max_ = max([regression_result[indexname].max() for indexname in subindicies[:-1]])
        min_ = min([regression_result[indexname].min() for indexname in subindicies[:-1]])
        divnorm = TwoSlopeNorm(vmin=min_, vcenter=0, vmax=max_)
        for i in range(N):
            data = regression_result[subindicies[i]]
            data = data.where(np.abs(data) > 0.01)
        ...
        fig.suptitle(f'{features[0].capitalize()} {features[1]} regression coefficients')
...
def contribution_to_trends(sic, prediction, indicies, ds, regression_results):
    ...
    data_m = prediction.polyfit(dim='time', deg=1) * 1e9*60*60*24*365
    seaice_gradient = sic.polyfit(dim='time', deg=1) * 1e9*60*60*24*365
    index_gradient = ds.polyfit(dim='time', deg=1) * 1e9*60*60*24*365
    for seaicename in sic:
        ...
        ax.coastlines()
        ax.set_axis_off()
        ax.set_title('Trend in SIC')
        cbar = plt.colorbar(contor)
        cbar.set_label('Trend in SIE (\% yr$^{-1}$)')
        features = seaicename.split('_')
        fig.suptitle(f'{features[0].capitalize()} {features[1]} seaice trends')
        
        gradient_prediction = regression_results[seaicename].copy() * 0

        total_prediction = data_m[seaicename+'_polyfit_coefficients'].sel(degree=1).copy() *0
        for index in gradient_prediction:
            if index != 'error': 
                gradient_prediction[index] = regression_results[seaicename][index] * index_gradient[index+'_polyfit_coefficients'].sel(degree=1)
                total_prediction += gradient_prediction[index]



        data = data_m[seaicename+'_polyfit_coefficients'].sel(degree=1)
        print(data)
        # data = total_prediction
        # print(data)
        data = data.where(abs(data) >=0.001)
        ...
        cbar = plt.colorbar(contor)
        cbar.set_label('Trend in SIE (\% yr$^{-1}$)')
        ax2.set_title('Predicted Trend in SIC')
        

        residual = seaice_m - data
        residual = residual.where(abs(residual) >=0.001)
...
def plot_contribution_timeseries(sic,prediction, ds, regression_results):
    ...
    for seaicename in sic:
        seaice_timeseries = (sic[seaicename]*area).sum(dim=('x','y'))/250
        predicted_timeseries = (prediction[seaicename]*area).sum(dim=('x','y'))/250
        ...
        plt.style.use('stylesheets/timeseries.mplstyle')
        fig  = plt.figure()
        ln1 = plt.plot(sic.time, seaice_timeseries,color = '#177E89', label = 'SIE')
        plt.plot(sic.time, linear_sic, color = '#177E89')
        ln2 = plt.plot(predicted_timeseries.time, predicted_timeseries, color = '#EA1B10', label='Prediction')
        plt.plot(predicted_timeseries.time, linear_predict, color = '#EA1B10')
...
def multiple_fast_regression(seaice, ds):
    ...
    for seaicename in seaice:
        ...
        params = fast_regression(new_indicies,sic)

        multiple_regressions = {index_names[i]:xr.DataArray(data = params[i], dims=('y','x'), coords = [sic.y, sic.x]) for i in range(len(params)-1)}
        multiple_regressions['error'] = xr.DataArray(data = params[-1], dims=('y','x'), coords = [sic.y, sic.x])
...
def main(subindicies, resolutions, temporal_resolution,temporal_decomposition,detrend):
    ...
    contribution_to_trends(sic, prediction, subindicies, ds, regression_results)
    plot_contribution_timeseries(sic,prediction, ds, regression_results)

def main2(subindicies, resolutions, temporal_resolution,temporal_decomposition,detrend):
    sic = load_seaice(resolutions, temporal_resolution, temporal_decomposition, detrend)
    ds = load_indicies(resolutions, temporal_resolution, temporal_decomposition, detrend)
    normalized = normalise_indicies(ds)[subindicies]
    regression_results = multiple_spatial_regression(sic, normalized)
    plot_coefficients(regression_results)
    ...
    for mode in regression_results.keys():
        for index in subindicies:
            prediction[mode] = prediction[mode] + regression_results[mode][index]*ds[index]
        prediction[mode] = prediction[mode] + regression_results[mode]['error']
    contribution_to_trends(sic, prediction, subindicies)
    plot_contribution_timeseries(sic,prediction, ds, regression_results)
